...
def add_alert_badges(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    scorecard.loc[scorecard["is_anomaly"], "alert_badge"] = "🔴 Anomaly: " + scorecard.loc[
        scorecard["is_anomaly"], "risk_tier"
    ].astype(str)
    return scorecard
...
def build_anomaly_detail_rows(
    ...
    for _, seller in scorecard[scorecard["is_anomaly"]].iterrows():
        metrics = [metric.strip() for metric in str(seller["anomalous_metrics"]).split(",") if metric.strip()]
